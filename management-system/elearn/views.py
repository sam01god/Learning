...
from .models import Blog, TakenQuiz, Profile, Quiz, Question, Answer, Learner, User, Course, Tutorial, Notes, Announcement, Msg
...
def dashboard(request):
    ...
    users = User.objects.all().exclude(username=request.user.username).count()
...
class CourcesList(LoginRequiredMixin, ListView):
    model = Course
    template_name = 'dashboard/admin/cources_list.html'
...
class StudentList(LoginRequiredMixin, ListView):
    model = User
    template_name = 'dashboard/admin/student_list.html'

    def get_queryset(self):
        return User.objects.filter(is_learner=True)

def studentdetail(request, id):
    user = get_object_or_404(User, id=id)
    profile = Profile.objects.filter(user=user)
    learner = Learner.objects.filter(user=user)
...
class TeacherList(LoginRequiredMixin, ListView):
    model = User
    template_name = 'dashboard/admin/teacher_list.html'
...
def teacherdetail(request, id):
    ...
    tutorial = Tutorial.objects.filter(user=user)
    note = Notes.objects.filter(user=user)
    context = {'profile': profile, 'tutorial': tutorial,
               'announcement': announcement, 'note': note}
...
class ListUserView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return User.objects.exclude(username=self.request.user.username).order_by('-id')
...
def home_instructor(request):
    note = Notes.objects.filter(user=request.user).count()
    instructor = Tutorial.objects.filter(user=request.user).count()
    course = Course.objects.all().count()
    users = Announcement.objects.filter(user=request.user).count()
    context = {'learner': note, 'course': course,
               'instructor': instructor, 'users': users}

    return render(request, 'dashboard/instructor/home.html', context)

class InsCourcesList(LoginRequiredMixin, ListView):
    model = Course
    template_name = 'dashboard/instructor/cources_list.html'
...
class LearnCourcesList(LoginRequiredMixin, ListView):
    model = Course
    template_name = 'dashboard/learner/cources_list.html'
...
def quizquestion(request, id):
    ...
    return render(request, 'dashboard/learner/takenquizquestion.html', {'quiz':quiz})
...
# Message  section Code goes
def homepage_student(request):
    users = User.objects.all().exclude(username=request.user.username)
    return render(request, 'dashboard/learner/chat-home.html', {'users': users})


def homepage_instructor(request):
    users = User.objects.all().exclude(username=request.user.username)
    return render(request, 'dashboard/instructor/chat-home.html', {'users': users})

def homepage_admin(request):
    users = User.objects.all().exclude(username=request.user.username)
    return render(request, 'dashboard/admin/chat-home.html', {'users': users})
...
def chatroom(request, pk):
    ...
    return render(request, "dashboard/message/chatroom.html", {"other_user": other_user, 'users': User.objects.all().exclude(username=request.user.username), "messages": messages})


@login_required
def ajax_load_messages(request, pk):
    other_user = get_object_or_404(User, pk=pk)
    messages = Msg.objects.filter(seen=False, receiver=request.user)
    message_list = [{
...
def add_blog(request):
    courses = Course.objects.only('id', 'name')
    context = {'courses': courses}
    return render(request, 'dashboard/instructor/add_blog.html', context)
...
class BlogList(ListView):
    model = Blog
    template_name = 'dashboard/instructor/list_blog.html'
...
def update_blog(request, pk):
    if request.method == 'POST':
        ...
        Blog.objects.filter(id=pk).update(title=title, content=content)
        messages.success = (request, 'Blog was updated successfully!')
        return redirect('list_blog')
    else:
        blog = Blog.objects.filter(id=pk)
        if request.user.is_instructor == True:
            return render(request, 'dashboard/instructor/blog_update.html', {'blog':blog})
        if request.user.is_learner == True:
            return render(request, 'dashboard/learner/blog_update.html', {'blog':blog})
...
def detail_blog(request, id):
    ...
    if request.user.is_instructor == True:
        return render(request, 'dashboard/instructor/blog_detail.html', {'blog':blog})
    if request.user.is_learner == True:
        return render(request, 'dashboard/learner/blog_detail.html', {'blog':blog})
